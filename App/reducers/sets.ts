...
type SetsAction = ActionType<typeof sets> | ActionType<typeof exercises>

const byId = (state: { [id: string]: Set } = {}, action: SetsAction) => {
  switch (action.type) {
...
const allIds = (state: string[] = [], action: SetsAction) => {
  switch (action.type) {
    case getType(sets.addSet):
